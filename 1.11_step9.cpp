...
поэтому он нашел словарь, в котором указано, как ставятся ударения в словах. К сожалению, в этом словаре присутствуют
не все слова. Вася решил, что в словах, которых нет в словаре, он будет считать, что Петя поставил ударения правильно,
...
Если есть несколько возможностей расстановки ударения в одном и том же слове, то эти варианты в словаре идут
...
int main()
{
	...
	for (int i = 0; i < n; ++i)
	{
		...
		{
			c = std::tolower(c); // Convert to lowercase for dictionary lookup
		}

		words[lowercaseWord].insert(word);
	}

	std::cin.ignore(); // Ignore newline after the number

	std::string line;
	std::getline(std::cin, line);

	std::istringstream iss(line); // Create a string stream to split the exercise
	...
	while (iss >> word)
	{
		...
		if (words.find(lowercaseWord) != words.end() && words[lowercaseWord].find(word) == words[lowercaseWord].end())
		{
			result++;
		}
		else if (std::count_if(word.begin(), word.end(), ::isupper) != 1)
		{
			result++;
		}
	}
